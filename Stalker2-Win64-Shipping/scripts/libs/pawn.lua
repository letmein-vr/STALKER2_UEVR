...
local function doHideForearmBones(val)
	local armsMesh = M.getArmsMesh()
	if armsMesh ~= nil then
		M.print("Hiding forearm bones: " .. tostring(val))
		if val then
			if getParameter("pawnForearmRight") ~= nil and getParameter("pawnForearmRight") ~= "" then
				armsMesh:HideBoneByName(uevrUtils.fname_from_string(getParameter("pawnForearmRight")), 0)
			end
			if getParameter("pawnForearmLeft") ~= nil and getParameter("pawnForearmLeft") ~= "" then
				armsMesh:HideBoneByName(uevrUtils.fname_from_string(getParameter("pawnForearmLeft")), 0)
			end
		else
			if getParameter("pawnForearmRight") ~= nil and getParameter("pawnForearmRight") ~= "" then
				armsMesh:UnHideBoneByName(uevrUtils.fname_from_string(getParameter("pawnForearmRight")))
			end
			if getParameter("pawnForearmLeft") ~= nil and getParameter("pawnForearmLeft") ~= "" then
				armsMesh:UnHideBoneByName(uevrUtils.fname_from_string(getParameter("pawnForearmLeft")))
			end
		end
	end
end
...
function M.hideForearmBones(val)
	saveParameter("hidePawnForearmBones", val)
end
...
local function executeIsForearmBonesHiddenCallback(...)
	return uevrUtils.executeUEVRCallbacksWithPriorityBooleanResult("is_forearm_bones_hidden", table.unpack({...}))
end
function M.registerIsForearmBonesHiddenCallback(func)
	uevrUtils.registerUEVRCallback("is_forearm_bones_hidden", func)
end
...
uevrUtils.setInterval(100, function()
	...
	isHidden, priority = executeIsForearmBonesHiddenCallback()
	if isHidden == nil then isHidden = getParameter("hidePawnForearmBones") end
	if pawnState.hideForearmBones ~= isHidden then
		doHideForearmBones(isHidden)
		pawnState.hideForearmBones = isHidden
	end
