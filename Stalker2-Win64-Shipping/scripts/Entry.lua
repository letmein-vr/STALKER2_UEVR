...
uevr.sdk.callbacks.on_pre_engine_tick(
    function(engine, delta)
        if gameState:IsLevelChanged(engine) then
            ...
            -- Only block item logic if Climbing or in UI
            if not _G.SuppressItemAttachment and not currentMenuState then
                -- Check for item changes only when not in menus/conversations
                if not gameState.isConversation then
                    local weaponMesh, weaponName, _ = gameState:GetEquippedWeapon()
                end
            end
...
local function saveWeaponProfile()
    if currentWeaponName then
        ...
        -- Save two-hand foregrip box config from two_hand module
        -- local thCfg = twoHand.get_config()
        -- profile.twoHandBox = {
        --     socket = thCfg.socket,
        --     scaleX = thCfg.scaleX, scaleY = thCfg.scaleY, scaleZ = thCfg.scaleZ,
        --     offX   = thCfg.offX,   offY   = thCfg.offY,   offZ   = thCfg.offZ,
        --     rotX   = thCfg.rotX,   rotY   = thCfg.rotY,   rotZ   = thCfg.rotZ,
        -- }
...
uevr.sdk.callbacks.on_draw_ui(function()
    ...
    imgui.separator()
    imgui.text("--- Mag Reload Collision Boxes ---")
    if imgui.tree_node("Mag Reload Box Sizes & Offsets") then
        ...
        if changedMagDebug then
            magReload.set_debug(newMagDebug)
        end
        imgui.separator()
        ...
        imgui.separator()
        imgui.text("Left Hand Box (rotation local)")
        local hrXC, newHRX = imgui.drag_float("Rot X (Roll)##hand", bc.handRotX or 0, 1.0, -180.0, 180.0)
        local hrYC, newHRY = imgui.drag_float("Rot Y (Pitch)##hand", bc.handRotY or 0, 1.0, -180.0, 180.0)
        local hrZC, newHRZ = imgui.drag_float("Rot Z (Yaw)##hand", bc.handRotZ or 0, 1.0, -180.0, 180.0)
        if hrXC then bc.handRotX = newHRX; bcChanged = true end
        if hrYC then bc.handRotY = newHRY; bcChanged = true end
        if hrZC then bc.handRotZ = newHRZ; bcChanged = true end
        ...
        if bcChanged then
            magReload.set_config(bc)
        end

        imgui.separator()
        local wname = currentWeaponName or "(no weapon)"
        imgui.text("Weapon: " .. wname)
        if imgui.button("Save Box Settings to Weapon Profile") then
            saveWeaponProfile()
            print("[MagReload] Saved box profile for: " .. wname)
        end
        imgui.same_line()
        if imgui.button("Reset to Defaults") then
            magReload.set_config({
    ...
    if imgui.tree_node("Gesture Zone Debug Coordinates") then
        local locGestures = require("gestures.locationgesture")
        
        local LtoR = locGestures.LeftHandRelativeToRightLocationGesture
        local RtoL = locGestures.RightHandRelativeToLeftLocationGesture
        
        imgui.text("Left Hand Relative to Right (RH Player):")
        if LtoR and LtoR.isActive then
            local p = LtoR.weaponLocation
            imgui.text(string.format("  X (Forward/Back): %.2f", p.x))
            imgui.text(string.format("  Y (Left/Right):   %.2f", p.y))
            imgui.text(string.format("  Z (Up/Down):      %.2f", p.z))
        else
            imgui.text("  (Inactive)")
        end
        ...
        imgui.text("Right Hand Relative to Left (LH Player):")
        if RtoL and RtoL.isActive then
            local p = RtoL.weaponLocation
            imgui.text(string.format("  X (Forward/Back): %.2f", p.x))
            imgui.text(string.format("  Y (Left/Right):   %.2f", p.y))
            imgui.text(string.format("  Z (Up/Down):      %.2f", p.z))
        else
        ...
        local changedVis, newVis = imgui.checkbox("Show UE Debug Boxes for Zones", isVis)
        if changedVis then
            zoneDebug.set_visible(newVis)
        end
    ...
    -- imgui.separator()
    -- imgui.text("--- Two-Hand Grip Box ---")
    -- if imgui.tree_node("Two-Hand Box Size & Offset") then
    --     local thc = twoHand.get_config()
    --     local thcChanged = false

    --     imgui.text("Attach Socket Name")
    --     local sockChanged, newSock = imgui.input_text("Socket##th", thc.socket)
    --     if sockChanged then thc.socket = newSock; thcChanged = true end

    --     imgui.separator()
    --     imgui.text("Foregrip Box (scale)")
    --     local tsX, nX = imgui.drag_float("Scale X##th", thc.scaleX, 0.01, 0.01, 5.0)
    --     local tsY, nY = imgui.drag_float("Scale Y##th", thc.scaleY, 0.01, 0.01, 5.0)
    --     local tsZ, nZ = imgui.drag_float("Scale Z##th", thc.scaleZ, 0.01, 0.01, 5.0)
    --     if tsX then thc.scaleX = nX; thcChanged = true end
    --     if tsY then thc.scaleY = nY; thcChanged = true end
    --     if tsZ then thc.scaleZ = nZ; thcChanged = true end

    --     imgui.separator()
    --     imgui.text("Foregrip Box (offset from socket)")
    --     local toX, noX = imgui.drag_float("Off X##th", thc.offX, 0.1, -50.0, 50.0)
    --     local toY, noY = imgui.drag_float("Off Y##th", thc.offY, 0.1, -50.0, 50.0)
    --     local toZ, noZ = imgui.drag_float("Off Z##th", thc.offZ, 0.1, -50.0, 50.0)
    --     if toX then thc.offX = noX; thcChanged = true end
    --     if toY then thc.offY = noY; thcChanged = true end
    --     if toZ then thc.offZ = noZ; thcChanged = true end

    --     imgui.separator()
    --     imgui.text("Foregrip Box (rotation local)")
    --     local rX, nrX = imgui.drag_float("Rot X (Roll)##th", thc.rotX, 1.0, -180.0, 180.0)
    --     local rY, nrY = imgui.drag_float("Rot Y (Pitch)##th", thc.rotY, 1.0, -180.0, 180.0)
    --     local rZ, nrZ = imgui.drag_float("Rot Z (Yaw)##th", thc.rotZ, 1.0, -180.0, 180.0)
    --     if rX then thc.rotX = nrX; thcChanged = true end
    --     if rY then thc.rotY = nrY; thcChanged = true end
    --     if rZ then thc.rotZ = nrZ; thcChanged = true end

    --     if thcChanged then
    --         twoHand.set_config(thc)
    --     end

    --     imgui.separator()
    --     local wname = currentWeaponName or "(no weapon)"
    --     imgui.text("Weapon: " .. wname)
    --     if imgui.button("Save Box Settings to Weapon Profile##th") then
    --         saveWeaponProfile()
    --         print("[TwoHand] Saved box profile for: " .. wname)
    --     end
    --     imgui.same_line()
    --     if imgui.button("Reset to Defaults##th") then
    --         twoHand.set_config({
    --             socket="jnt_l_hand",
