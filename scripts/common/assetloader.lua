...
local kismet_string_library = utils.find_static_class("Class /Script/Engine.KismetStringLibrary")
local kismet_system_library = utils.find_static_class("Class /Script/Engine.KismetSystemLibrary")

local function get_struct_object(structClassName)
	local class = utils.find_required_object(structClassName)
	if class ~= nil then
...
local function find_first_of(className, includeDefault)
	...
	local class =  utils.find_required_object(className)
	if class ~= nil then
