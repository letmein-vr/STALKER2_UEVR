...
function ScopeController:attach_components_to_weapon(weapon_mesh)
    ...
    if self.scope_plane_component then
        ...
        -- Glitch Fix for Holo Scopes (Skip PIP + Shader Fix)
        if self.scope_mesh then
             local name = self.scope_mesh:get_fname():to_string():lower()
             if name:find("holo") or name:find("deadeye_scope") or name:find("collimator") or name:find("goloscope") then  
                 print("[Scope] Detected Holo/Collimator/Golo: " .. name .. " - DESTROYING PIP and patching material.")
                 
                 -- CRITICAL FIX FOR UPSCALING (DLSS/FSR) ARTIFACTS
                 -- SceneCaptureComponent conflicts with Upscalers in VR. We must DESTROY it.
                 if self.scene_capture_component then
                     ...
                     self.scene_capture_component = nil -- Clear reference
                 end

                 -- Apply Safer Material Fix (Disable Refraction/Specular)
                  -- We avoid changing the Root Material (causes global corruption).
                  -- We avoid DMI Opacity (hides Reticle).
                  -- We just flatten the optical properties to remove VR distortion glitches.
                  
                  -- Scan materials to confirm we should apply fix
                  local shouldFix = true -- Apply to all Holo/Collimator scopes detected above
                  
                  if shouldFix then
                      print("[Scope] Applying Global Refraction Fix to: " .. name)
                      pcall(function()
                          self.scope_mesh:SetScalarParameterValueOnMaterials("Refraction", 0.0)
                          self.scope_mesh:SetScalarParameterValueOnMaterials("Specular", 0.0)
                          self.scope_mesh:SetScalarParameterValueOnMaterials("Roughness", 0.0)
                          self.scope_mesh:SetScalarParameterValueOnMaterials("Metallic", 0.0)
                          self.scope_mesh:SetScalarParameterValueOnMaterials("SightMaskScale", 0.0) -- Ensure masking is off/neutral
                      end)
                  end
                 
                 -- Disable Scope System (prevent masking and PIP creation for this mesh)
                 self.scope_mesh = nil 
                 -- return (Removed to allow processing of other meshes in the loop below)
             end
        end
        
        -- Get ALL scope meshes to ensure we mask everything (lens caps, glass, etc.)
        -- AND Check if ANY of them are Holo/Collimator to trigger SceneCapture destruction
        local all_scope_meshes = GameState:get_all_scope_meshes(weapon_mesh)
        local is_holo_or_collimator = false
        
        -- Brute Force Check: Scan ALL attached components for "colimscope" or "holo" if GameState missed it
        local all_components = weapon_mesh.AttachChildren 
        -- Note: AttachChildren property returns an array of SceneComponents. 
        -- We might need to check the Owner's components if they aren't direct children of the mesh? 
        -- Usually attachments are children of the WeaponMesh.
        
        if all_components then
             for i = 1, #all_components do
                 local comp = all_components[i]
                 if comp and comp.StaticMesh then -- Check if it's a static mesh component
                     local name = comp:get_fname():to_string():lower()
                     if name:find("holo") or name:find("deadeye_scope") or name:find("collimator") or name:find("goloscope") or name:find("colimscope") then
                         is_holo_or_collimator = true
                         print("[Scope] Detected Holo/Collimator via Brute Force: " .. name)
                         -- Apply Material Fix
                         pcall(function()
                              comp:SetScalarParameterValueOnMaterials("Refraction", 0.0)
                              comp:SetScalarParameterValueOnMaterials("Specular", 0.0)
                              comp:SetScalarParameterValueOnMaterials("Roughness", 0.0)
                              comp:SetScalarParameterValueOnMaterials("Metallic", 0.0)
                              comp:SetScalarParameterValueOnMaterials("SightMaskScale", 0.0)
                              comp:SetScalarParameterValueOnMaterials("PixelDepthOffset", 0.0)
                              comp:SetScalarParameterValueOnMaterials("DepthOffset", 0.0)
                         end)
                         -- Component Level Rendering Fixes
                         pcall(function()
                 ...
                 -- Also check for rogue SceneCaptureComponents while we are here!
                 if comp:is_a(self.scene_capture_component_c) then
                      print("[Scope] Found ROGUE SceneCaptureComponent2D attached to weapon! Name: " .. comp:get_fname():to_string())
                      -- We will destroy this later if is_holo_or_collimator is true
                 end
             end
        end
        ...
        if all_scope_meshes then
            for _, mesh in ipairs(all_scope_meshes) do
                ...
                -- Check for Holo/Collimator keywords
                if name:find("holo") or name:find("deadeye_scope") or name:find("collimator") or name:find("goloscope") then
                     is_holo_or_collimator = true
                     print("[Scope] Detected Holo/Collimator in list: " .. name .. " - Applying Immediate Fix.")
                     pcall(function()
                          ...
                          mesh:SetScalarParameterValueOnMaterials("PixelDepthOffset", 0.0)
                          mesh:SetScalarParameterValueOnMaterials("DepthOffset", 0.0)
                     end)
                     -- Component Level Rendering Fixes
                     pcall(function()
                     ...
                     -- Standard masking for other scope parts
                     -- print("Masking scope mesh: " .. name)
                     mesh:SetScalarParameterValueOnMaterials("SightMaskScale", 0.0)
                end
            end
        end

        -- If ANY mesh was a Holo/Collimator, fallback to destroy logic even if main scope wasn't detected as one
        if is_holo_or_collimator or (self.scope_mesh and (self.scope_mesh:get_fname():to_string():lower():find("holo") or self.scope_mesh:get_fname():to_string():lower():find("collimator"))) then
             print("[Scope] Holo/Collimator Logic Triggered - DESTROYING PIP.")
             
             -- 1. Destroy our managed component
             if self.scene_capture_component then
                 if self.scene_capture_component.K2_DestroyComponent then
                     self.scene_capture_component:K2_DestroyComponent(self.scene_capture_component)
                 else
             ...
             -- 2. Clean up any ROGUE SceneCaptureComponents on the weapon (from game or previous script runs)
             local children = weapon_mesh.AttachChildren
             if children then
                 for i = 1, #children do
                     local child = children[i]
                     if child:is_a(self.scene_capture_component_c) then
                         print("[Scope] Destroying ROGUE SceneCapture: " .. child:get_fname():to_string())
                         if child.K2_DestroyComponent then child:K2_DestroyComponent(child) end
                     end
                 end
             end

             self.scope_mesh = nil
             -- We do NOT return here, we allow the rest of the function to clean up or do other fixes if needed, 
             -- but since scope_mesh is nil, the PIP plane attachment below will check for validity.
        end
        ...
        -- OpticCutoutSocket
        -- self.scope_mesh:SetScalarParameterValueOnMaterials("SightMaskScale", 0.0) -- Already handled above
...
function ScopeController:Update(engine)
    ...
    if weapon_mesh then
        ...
        local scope_changed = (not is_mesh_valid or (is_mesh_valid and not self.scope_mesh.AttachParent)) and GameState:is_scope_active(c_pawn)
